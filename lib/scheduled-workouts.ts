 ...
 * string). Verifies the workout is owned by `userId` before inserting —
 * mirrors createSessionForWorkout's ownership check in lib/sessions.ts —
 * so a forged workoutId belonging to another user can't be scheduled.
 * Returns null if the workout doesn't exist or isn't owned by `userId`, in
 * which case no row is created.
 */
export async function scheduleWorkoutForUser(
  ...
  const [created] = await db
    ...
    .values({ userId, workoutId: workout.id, scheduledDate, notes })
    .returning();
 ...
 * with its workout (and that workout's tags) nested. Used by the Upcoming
 * list on /schedule and the home page. `today` is computed in Postgres via
 * `current_date` rather than passed in from the application server, so it
 * can't drift from the database's own notion of "today."
 */
 ...
 * skipped/completed/planned entries differently.
 */
 ...
 * completed instead of leaving it dangling. "Earliest" breaks ties between
 * same-day entries by created_at. The candidate row is selected and
