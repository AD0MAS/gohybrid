 ...
 * GoalsList/EventsList/BodyMetricsList/PersonalRecordsList within the same
 * request) and hands them to GoalFields — the actual form is a client
 * component because useActionState (for inline validation-error display)
 * and the goal-type-driven field visibility both require one; this wrapper
 * stays a Server Component so the data fetching itself doesn't need to move
 * to the client. Rendered from page.tsx's own header rather than from
 * GoalsList: Goals is the one section whose Add action is promoted to the
 * page's single accent call-to-action (every other section's Add stays a
 * quiet trigger inside its own panel, fetched by that section directly).
 ...
 * PersonalRecordsList's own fetch, for the same "previously used" group in
