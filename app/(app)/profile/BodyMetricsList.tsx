 ...
 * newest-measured-first, so grouping here doesn't need to re-sort.
 */
export default async function BodyMetricsList() {
  const user = await requireUser();
  const metrics = await getBodyMetricsForUser(user.id);
  ...
  return (
    <div className="flex flex-col gap-4">
      {bodyMetricTypeEnum.enumValues.map((type) => {
        ...
        const { label, unit } = BODY_METRIC_LABELS[type];
            ...
            <ul className="flex flex-col gap-2">
              {entries.map((entry) => (
                ...
                >
                  <div>
                    <p className="text-sm">
                      {entry.value} {unit} · {entry.measuredAt}
                    </p>
                    {entry.notes && (
